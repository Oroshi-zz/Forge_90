...
| | |
|---|---|
| **Workout plan** — phases, sessions and rotation by week<br>![Workout plan](docs/screenshots/workout-plan.png) | **Exercise library** — form tips, on/off switches, your own exercises<br>![Exercise library](docs/screenshots/exercise-library.png) |
...
| | |
|---|---|
| **Today · Log sets** — weight × reps per set, live counter, PR badges, next-time targets<br>![Log sets on the dashboard](docs/screenshots/dashboard-log.png) | **Quick edit** — change a day's session or meals from the dashboard<br>![Quick edit](docs/screenshots/quick-edit.png) |
...
*Diet plan — targets, portion scaling, trend coach*
...
| | |
|---|---|
| **Food preferences** — group › subgroup › single food<br>![Food preferences](docs/screenshots/food-preferences.png) | **Food editor** — macros, package size, reset to defaults<br>![Food editor](docs/screenshots/food-editor.png) |
...
![Grocery](docs/screenshots/grocery.png)
*Grocery & money saver*
...
4. **Shared shopping:** the grocery list, batch-cook schedule and cook notes include **both people's portions** of every shared meal ("COOK ×4 — 2 for you, 2 for your partner"), and checking an item off checks it off for both. Everyone's portions are still sized to their own calories and protein.
...
| **Shopping for two** — combined list and batch cooking<br>![Shared grocery list](docs/screenshots/grocery-shared.png) |  |
...
*Progress — weight, body fat, lean mass, PRs*
...
| **Forgot password**<br>![Forgot password](docs/screenshots/forgot-sent.png) | **Password-reset email** — 30-minute, single-use link<br>![Reset email](docs/screenshots/email-reset.png) |
...
| **Security** | Minimum password length and complexity, failed attempts before lockout and how long it lasts, emailing a reset link on lockout, session length, "keep me signed in" length, password-change emails, and **Require HTTPS**. |
| **App settings** | App name, used in emails and in the browser tab (e.g. "Home Gym · Dashboard"), the app address used in invite and reset links, and the default theme for new accounts. |
| **Server & proxy** | Live checks of this connection: HTTPS detected, the visitor IP it sees, app address, secure cookies, Require HTTPS. Also shows what the server received (`X-Forwarded-*` headers, `TRUST_PROXY`) and your Nginx Proxy Manager setup with your own values filled in. |
...
| **Admin · Server & proxy** — connection checks behind Nginx Proxy Manager<br>![Admin server and proxy](docs/screenshots/admin-proxy.png) | |
...
<p align="center"><i>On a phone: sign-in, dashboard and calendar agenda</i></p>
...
FORGE 90 is published to GitHub Container Registry:

```
ghcr.io/oroshi-zz/forge_90:latest
```

Versioned tags such as `:v1.0` are published too. Unraid pulls the image when you add the container, so there's nothing to download first.
...
| `TRUST_PROXY` | *(off)* | Behind a reverse proxy: its IP address or Docker subnet (e.g. `172.18.0.0/16`, comma-separated for several), or `true` to trust any. See [HTTPS & reverse proxies](#https--reverse-proxies). |
| `REQUIRE_HTTPS` | *(auto)* | Set to `false` to switch off the redirect from plain HTTP to your https `APP_URL`, e.g. if you're locked out. |
...
Google no longer accepts an account's normal password over SMTP. Invites and password resets are both sent by email, so **set this up before inviting anyone**. To send from a Gmail account:
...
FORGE 90 is meant to run on its own subdomain behind a reverse proxy that handles HTTPS and the certificate (Nginx Proxy Manager, SWAG, Traefik or a Cloudflare Tunnel). It has no built-in SSL and no URL-base option.

**Nginx Proxy Manager:**

1. **Add a proxy host:**
   - domain `forge.yourdomain.com`, scheme `http`
   - forward hostname/IP: the FORGE 90 container (its name or IP), port `8090`
   - turn on **Block Common Exploits**
2. **SSL tab:** pick your certificate (or request a Let's Encrypt one) and turn on **Force SSL**, **HTTP/2** and **HSTS**.
3. **Unraid template:**
   - `APP_URL=https://forge.yourdomain.com`, so email links use the public address and plain-HTTP visits get redirected there
   - `TRUST_PROXY=` your proxy's IP address or Docker subnet, e.g. `172.18.0.0/16`, so FORGE 90 sees visitors' real IPs and knows the connection is HTTPS
4. **Best:** put FORGE 90 and Nginx Proxy Manager on the same **custom Docker network** and remove FORGE 90's port mapping, so it can only be reached through the proxy.
5. Open **Admin → Server & proxy** through your https address. Every check should be green, and anything left to fix comes with the exact setting to change.

**How FORGE 90 handles the proxy:**

- **Trusted proxies:** `X-Forwarded-For` and `X-Forwarded-Proto` are only believed when the connection comes from an address in `TRUST_PROXY`.
  - The visitor's IP is read from the right of `X-Forwarded-For`, the part your proxy adds, so entries a visitor adds themselves can't fake it.
  - `TRUST_PROXY=true` trusts any connecting address. That works, but anyone on your network could fake their IP, so a subnet is better.
- **Require HTTPS** (on by default once `APP_URL` starts with `https://`, and switchable in **Admin → Security**):
  - Plain-HTTP visits to any other address, such as `http://<unraid-ip>:8090`, are redirected to your https address.
  - API calls over plain HTTP are refused.
  - Browsers are told to always use HTTPS (HSTS).
  - The health check and requests from inside the container are exempt.
  - If your https hostname itself arrives as plain HTTP (a proxy misconfiguration), FORGE 90 serves it rather than looping, and Server & proxy flags it.
  - **Can't get in** because the domain or proxy is down? Set `REQUIRE_HTTPS=false` in the template to turn the redirect off.
- **Secure cookies:** session cookies are marked `Secure` automatically over HTTPS.
...
The Docker build runs `node build.js` and copies the server into a small Node 22 Alpine image. There are no npm packages. Every push to `main` makes the GitHub Actions workflow (`.github/workflows/docker-publish.yml`) build and publish the image to GitHub Container Registry.

**Using your own fork:**

- The workflow publishes to `ghcr.io/<your-github-user>/<repo-name>`. Use that as the *Repository* in your container template.
- New packages on GitHub are private. Either make the package public (**Packages → Package settings → Change visibility**), or run `docker login ghcr.io` on the Docker host with a personal access token that has `read:packages`.

**Releasing a new version:** change the number in `VERSION` and push. The new version shows in the side menu, on Settings, under Admin → Data & backup, and at `/api/health`. To also publish a versioned image, push a matching git tag (`git tag v1.0 && git push origin v1.0`); GitHub Actions then publishes a `:v1.0` image alongside `latest`.
...
- **Behind a proxy:** forwarded headers are only trusted from the addresses in `TRUST_PROXY`, and the visitor IP comes from the entry your proxy added, so per-IP limits and the activity log can't be fooled with a fake header. With Require HTTPS on, plain-HTTP visits are redirected and HSTS is sent.
