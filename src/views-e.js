...
function admUserModal(id) {
  ...
  modal(`<div class="adm-user-modal"><div class="row" style="align-items:flex-start">${avatarHTML(u, 'lg')}<div style="flex:1;min-width:0"><h2>${esc(u.name)}${me ? ' <span class="pill">You</span>' : ''}</h2><div class="small muted">${esc(u.email)}</div><div class="row wrap" style="gap:6px;margin-top:6px"><span class="pill ${u.role === 'admin' ? 'acc' : ''}">${u.role === 'admin' ? 'Administrator' : 'Member'}</span>${statusPill(u)}</div></div><button class="btn icon ghost" data-act="close-modal">${icon('x')}</button></div>
    ...
    ${u.avatarUrl && !me ? `<h3>Profile picture</h3><div class="adm-acts"><button class="btn" data-act="adm-avatar-rm" data-id="${id}">${icon('trash')}Remove profile picture</button></div>` : ''}
