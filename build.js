...
const js=`const APP_VERSION = ${JSON.stringify(VERSION)};\n`+['exercises','nutrition','foods-db','recipes','engine','ui-core','views-a','views-b','views-c','views-d','views-e','views-f','views-g','views-h','views-i','views-j','views-k','views-l','views-m','views-n','main'].map(f=>`/* ==== ${f}.js ==== */\n`+strip(fs.readFileSync('src/'+f+'.js','utf8'))).join('\n');
